...
void* fc2_output_stage(void* arg) {
    for (int i = 0; i < NUM_INPUTS; i++) {
        ...
        offset += snprintf(buf + offset, sizeof(buf) - offset, "Input Patch [0:3][0:3][0]:\n");
        for (int x = 0; x < 3; x++) {
            for (int y = 0; y < 3; y++)
                offset += snprintf(buf + offset, sizeof(buf) - offset, "%.1f ", s->input[x][y][0]);
            offset += snprintf(buf + offset, sizeof(buf) - offset, "\n");
        }
        offset += snprintf(buf + offset, sizeof(buf) - offset, "Conv Output [0:5][0][0] = ");
        for (int j = 0; j < 5; j++) offset += snprintf(buf + offset, sizeof(buf) - offset, "%.2f ", s->conv_output[j][0][0]);
        offset += snprintf(buf + offset, sizeof(buf) - offset, "\nfc1[0:5] = ");
        for (int j = 0; j < 5; j++) offset += snprintf(buf + offset, sizeof(buf) - offset, "%.2f ", s->fc1_output[j]);
        offset += snprintf(buf + offset, sizeof(buf) - offset, "\nfc2[0:5] = ");
        for (int j = 0; j < 5; j++) offset += snprintf(buf + offset, sizeof(buf) - offset, "%.2f ", s->fc2_output[j]);
        offset += snprintf(buf + offset, sizeof(buf) - offset, "\n");
        fputs(buf, stdout);
