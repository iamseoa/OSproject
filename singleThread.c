...
void fc1_forward(CNNModel* model, float input[], float output[FC1_OUT]) {
    pthread_t tid;
    FCThreadArg arg = {0, FC1_OUT, model, input, output};
    pthread_create(&tid, NULL, fc1_thread_fn, &arg);
    pthread_join(tid, NULL);
}
...
void fc2_forward(CNNModel* model, float input[], float output[FC2_OUT]) {
    pthread_t tid;
    FCThreadArg arg = {0, FC2_OUT, model, input, output};
    pthread_create(&tid, NULL, fc2_thread_fn, &arg);
    pthread_join(tid, NULL);
}
