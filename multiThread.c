...
void* fc1_thread_fn(void* arg) {
    FCThreadArg* a = (FCThreadArg*)arg;
    for (int i = a->start_i; i < a->end_i; i++) {
        a->output[i] = a->model->fc1.biases[i];
        for (int j = 0; j < (CONV_OUT/2)*(CONV_OUT/2)*CONV_DEPTH; j++)
            a->output[i] += a->model->fc1.weights[i][j] * a->input[j];
    }
    return NULL;
}

void fc1_forward(CNNModel* model, float* input, float* output) {
    pthread_t threads[NUM_THREADS_FC1];
    FCThreadArg args[NUM_THREADS_FC1];
    int chunk = FC1_OUT / NUM_THREADS_FC1;
    for (int i = 0; i < NUM_THREADS_FC1; i++) {
        args[i] = (FCThreadArg){i*chunk, (i+1)*chunk, model, input, output};
        pthread_create(&threads[i], NULL, fc1_thread_fn, &args[i]);
    }
    for (int i = 0; i < NUM_THREADS_FC1; i++)
        pthread_join(threads[i], NULL);
}

void* fc2_thread_fn(void* arg) {
    FCThreadArg* a = (FCThreadArg*)arg;
    for (int i = a->start_i; i < a->end_i; i++) {
        a->output[i] = a->model->fc2.biases[i];
        for (int j = 0; j < FC1_OUT; j++)
            a->output[i] += a->model->fc2.weights[i][j] * a->input[j];
    }
    return NULL;
}

void fc2_forward(CNNModel* model, float* input, float* output) {
    pthread_t threads[NUM_THREADS_FC2];
    FCThreadArg args[NUM_THREADS_FC2];
    int chunk = FC2_OUT / NUM_THREADS_FC2;
    for (int i = 0; i < NUM_THREADS_FC2; i++) {
        args[i] = (FCThreadArg){i*chunk, (i+1)*chunk, model, input, output};
        pthread_create(&threads[i], NULL, fc2_thread_fn, &args[i]);
    }
    for (int i = 0; i < NUM_THREADS_FC2; i++)
        pthread_join(threads[i], NULL);
...
void* conv_thread_fn(void* arg) {
    ConvThreadArg* a = (ConvThreadArg*)arg;
    for (int d = a->start_d; d < a->end_d; d++) {
        for (int i = 0; i < CONV_OUT; i++) {
            for (int j = 0; j < CONV_OUT; j++) {
                float sum = a->model->conv.biases[d];
                for (int c = 0; c < CHANNELS; c++)
                    for (int ki = 0; ki < KERNEL_SIZE; ki++)
                        for (int kj = 0; kj < KERNEL_SIZE; kj++) {
                            int in_idx = ((i + ki) * INPUT_SIZE + (j + kj)) * CHANNELS + c;
                            sum += a->model->conv.weights[d][c][ki][kj] * a->input[in_idx];
                        }
                int out_idx = (i * CONV_OUT + j) * CONV_DEPTH + d;
                a->output[out_idx] = sum;
            }
        }
    }
    return NULL;
...
void print_resource_usage(struct timeval start, struct timeval end, double conv_t, double relu_t, double pool_t, double fc1_t, double fc2_t) {
    ...
    double cpu_util = ((user_cpu + sys_cpu) / wall_time) * 100.0;
...
int main() {
    ...
    for (int i = 0; i < NUM_INPUTS; i++) {
        ...
        gettimeofday(&t0, NULL); conv_forward(&model, inputs[i], model.conv_output); gettimeofday(&t1, NULL); conv_time += timer_ms(t0, t1);
        gettimeofday(&t0, NULL); relu_forward(model.conv_output, model.relu_output); gettimeofday(&t1, NULL); relu_time += timer_ms(t0, t1);
        gettimeofday(&t0, NULL); maxpool_forward(model.relu_output, model.pooled_output); gettimeofday(&t1, NULL); pool_time += timer_ms(t0, t1);
        flatten(model.pooled_output, flat);
        gettimeofday(&t0, NULL); fc1_forward(&model, flat, model.fc1_output); gettimeofday(&t1, NULL); fc1_time += timer_ms(t0, t1);
        gettimeofday(&t0, NULL); fc2_forward(&model, model.fc1_output, model.fc2_output); gettimeofday(&t1, NULL); fc2_time += timer_ms(t0, t1);
