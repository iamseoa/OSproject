...
void* consumer(void* arg) {
    while (1) {
        ...
        double wall_msec = (end.tv_sec - start.tv_sec) * 1e3 + (end.tv_nsec - start.tv_nsec) / 1e6;
...
int main() {
    ...
    *task_done_count = 0;
    ...
    clock_gettime(CLOCK_MONOTONIC, &end);
    struct rusage usage_self_end, usage_child_end;
    getrusage(RUSAGE_SELF, &usage_self_end);
    getrusage(RUSAGE_CHILDREN, &usage_child_end);

    double user_usec = (usage_self_end.ru_utime.tv_sec * 1e6 + usage_self_end.ru_utime.tv_usec) +
                       (usage_child_end.ru_utime.tv_sec * 1e6 + usage_child_end.ru_utime.tv_usec);
    double sys_usec = (usage_self_end.ru_stime.tv_sec * 1e6 + usage_self_end.ru_stime.tv_usec) +
