...
typedef struct {
    ...
    int out_channels;
    int kernel_size;
    double weights[16][3][3][3]; // [out_channels][in_channels][k][k]
} Conv2DLayer;
...
typedef struct {
    int input_size;
    int output_size;
    double weights[FC1_INPUT_SIZE][FC1_SIZE];
    double bias[FC1_SIZE];
} FullyConnectedLayer;
...
// Fully Connected
void fc_forward(FullyConnectedLayer* layer, double* input, double* output);
    ...
    for (int f = 0; f < 16; f++)
        for (int i = 0; i < VALID_SIZE; i += 2)
            for (int j = 0; j < VALID_SIZE; j += 2) {
                ...
                max_val = fmax(max_val, input[f][i+1][j]);
                max_val = fmax(max_val, input[f][i][j+1]);
                max_val = fmax(max_val, input[f][i+1][j+1]);
